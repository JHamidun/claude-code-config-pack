...
> Пошаговые сценарии фаз под Яндекс + RU. Везде сначала читай контекст из `context/product.md`.

## research — бриф под тему

1. Прочитать `context/target-keywords.md` + `context/product.md`.
2. Wordstat (см. `references/wordstat-real-recipe.md`): собрать частотность по теме, расширения, вопросы (что/как/почему).
3. Яндекс.Вебмастер: текущие позиции своих страниц по теме.
...
1. Бриф + контекст-голос (`context/product.md` → раздел «Голос бренда»).
...
1. Яндекс.Вебмастер: индексация, позиции, запросы, ошибки.
...
4. CRO-аудит — отдельный чеклист конверсии посадочной.
5. Публикация — ваш CMS (редактор страниц).

## publish — публикация

- Пост в блог/медиа → ваш CMS (Tilda Feeds API / WordPress REST / Headless).
- Лендинг → редактор страниц вашего CMS.
- Проставить мета (title/description/og) в SEO-полях страницы — см. `data-yandex-tilda.md`.
- После публикации: добавить в Яндекс.Вебмастер на переобход.
