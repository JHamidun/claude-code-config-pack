...
|------|---------|-----------|-----------|-------|
| **1. Файловая (курируемая)** | MEMORY.md индекс + topic-файлы, bi-temporal frontmatter | `~/.claude/projects/<project>/memory/*.md` | Read/Write/Edit + команды `/memory-*` | Каноничные решения, root cause, паттерны, решения пользователя — то, что должно жить долго и читаться человеком |
...
```bash
python ~/.claude/tools/search_chats.py index    # слой 3: собрать chats.db из своих сессий
python ~/.claude/scripts/memory_graph.py build  # слой 2: граф из заметок (пока их нет — 0 узлов)
```

Слой 1 создаётся первой же записью: `Write` topic-файла в `~/.claude/projects/<project>/memory/`
...
Директория: `~/.claude/projects/<project>/memory/`
...
памяти обходят `~/.claude/projects/*` целиком.
