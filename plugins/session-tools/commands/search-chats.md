...
Claude Code генерирует каждому чату короткое человекочитаемое имя («Найти и суммировать встречу
с юристом и бухгалтером», «Find Axsoft meeting transcript and summarize»). **Именно его владелец
...
python ~/.claude/tools/search_chats.py titles "юрист"
...
```
/search-chats titles юрист              # найти чат по ИМЕНИ (как в /resume)
...
## Реальный кейс (30.07.2026)

Владелец: «мы же собирали кучу документов по РИД — в этом ли чате? найди его название».
Полнотекстовый поиск давал message-id, но не имя. Через `ai-title` нашлось сразу:
**«Найти и суммировать встречу с юристом и бухгалтером»** (`9b2505cf-2c00-4a93-9b41-c8a66cd2f706`,
21–27.07, 331 сообщение). Проверка механики — на текущем чате: отдало ровно тот заголовок,
что владелец видел в интерфейсе.
