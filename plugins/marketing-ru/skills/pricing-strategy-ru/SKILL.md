...
metadata:
  ...
  reuses: performance-analytics, yandex, competitive-analysis, ab-testing-ru
---
...
**Перед началом собери контекст** своих продуктов и реальных цен (не выдумывай — недостающее помечай `[TODO]`). Пример линейки для рассуждений (подставь свои значения): подписка в схеме good-better-best (Basic / Plus / Pro в ₽/мес, годовая со скидкой, founding-цена первым N подписчикам, trial без карты); разовый флагман (Solo / Group / 1:1); add-on-сервис; воркшоп и консалтинг (цены индивидуальные).

## Перед стартом — собрать контекст

1. **Бизнес-контекст:** что ценим — подписка (Basic/Plus/Pro), разовый флагман, корпоративный воркшоп, консалтинг (вход = бесплатная консультация), B2B-когорты, add-on. GTM: self-serve B2C (trial подписки) + sales-led B2B (воркшоп/консалтинг).
2. **Ценность и конкуренты:** ключевая ценность (библиотека материалов, симуляторы с AI-тьютором, доступ к моделям включён, автор-практик), альтернативы (другие курсы, бесплатные туториалы, корп-тренинги).
3. **Текущий перформанс:** конверсия trial→paid `[TODO]`, ARPU/churn по мере набора данных, фидбэк по цене.
4. **Цели:** первые продажи из имеющегося трафика; рост подписки + апселл воркшопа/консалтинга B2B.
