---
name: paywall-cro-ru
description: "In-app пейволлы, экраны апгрейда, апселлы и фиче-гейты — конверсия trial→платящий, апгрейд тарифов. Триггеры: «пейволл», «paywall», «экран апгрейда», «feature gate», «лимит достигнут», «trial to paid», «trial не апгрейдятся», «экран окончания триала». НЕ: публичная страница тарифов→page-cro-ru; структура цен→pricing-strategy-ru; cancel-флоу/save-офферы→churn-prevention-ru."
metadata:
  version: 1.1.0
  updated: 2026-05-29
  ported_from: coreyhaines31/marketingskills (paywalls)
  reuses: web-analytics, ab-testing-ru
---
...
**Перед началом собери контекст своего продукта:** структура тарифов и цены (месяц/год, trial-период, спец-офферы, разовые продукты, add-ons), карта воронки. Не дублируй цены и метрики.
...
- **Trial→paid конверсия, показы/клики пейволла, сегменты, revenue, апгрейды тарифов** → `web-analytics` (Метрика/GA) + продуктовая БД; платежи — вебхуки платёжного провайдера.
...
| Нужно | Скилл |
|------|-------|
| Данные конверсии/revenue | `web-analytics` |
