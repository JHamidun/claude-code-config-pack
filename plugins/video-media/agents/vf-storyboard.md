...
2. Схема конверта: `~/.claude/skills/video-montage/schemas/production.schema.json` —
   там перечислены допустимые значения ролей, крупностей и движений.
3. Каталог приёмов: `python ~/.claude/skills/video-montage/scripts/techniques.py list --cat camera`
