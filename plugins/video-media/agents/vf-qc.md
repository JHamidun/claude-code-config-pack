...
```bash
python ~/.claude/skills/video-montage/scripts/techniques.py list --cat qa
python ~/.claude/skills/video-montage/scripts/techniques.py for-block крючок
```
...
Визуальное проверяется **глазами на кадрах**, а не рассуждением по коду. Доставай кадры
и смотри их:
