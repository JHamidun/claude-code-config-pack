...
- **B1 — Cutout parallax rig.** Take the hero image, cut the subject out (prompt
  it on a flat chroma ground, key it in the sandbox), generate a clean
  background plate behind it (+ optionally a mid layer: fog, foliage,
  particles as transparent PNGs). Stack 3-5 layers
...
- **B2 — Grade-shift interaction pair.** Two renders of the SAME composition
  ...
  effect.
- **B3 — Deep parallax scene.** B1 pushed to spectacle tier: 5+ keyed layers,
  scroll-driven camera-style translation plus cursor tilt, and a subtle
  per-layer blur ramp for depth of field. This is the substitute for a real 3D
  subject, which this surface cannot generate — do not promise the user a mesh,
  an orbit, or a model they can spin.
