<!-- LEGACY: полное тело скилла 'version-snapshots' из старого дерева ${WORKSPACE}/tools/claude-code-skills (@2026-04-30).
...
description: История версий артефакта — снимок при каждом save, можно откатиться, сравнить два моментa. Не git (тяжело для прототипа), а лёгкая sidecar папка `.snapshots/`.
