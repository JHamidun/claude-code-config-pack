#!/usr/bin/env python3
"""Рабочая почта компании через УСТАНОВЛЕННЫЙ Outlook (COM), а не по сети.

Команда `/outlook` полтора года описывала сетевой путь через `exchangelib` на
mail.company.example. Он не работает: падает `RecursionError` в urllib3 ещё до авторизации,
воспроизводится стабильно, пароль тут ни при чём. Тратить на него попытки не нужно.

Работает обращение к локальному Outlook: приложение стоит на машине, учётная запись
`you@company.example` в нём уже настроена, пароль не требуется. Проверено на
живом ящике — 5166 писем во входящих. Если Outlook закрыт, COM запустит его сам.
...
Нужен pywin32 (стоит) и установленный Outlook (стоит).
"""
