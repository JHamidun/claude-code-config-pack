...
| `~/.claude/.gmail-tokens/*.json` | `gmail.readonly` + `gmail.modify` | Gmail: чтение, поиск, **отправка**. По файлу на ящик — сколько заведёшь, столько и работает |
| `~/.claude/google_service_account.json` | служебный ящик вида `<имя>@<проект>.iam.gserviceaccount.com` | таблицы и файлы, расшаренные на робота |
...
Инструменты лежат в `~/.claude/tools/`, навык на них ссылается. Ящиков может быть один
или два десятка: каждый — свой файл в `~/.claude/.gmail-tokens/`, `--list-accounts`
