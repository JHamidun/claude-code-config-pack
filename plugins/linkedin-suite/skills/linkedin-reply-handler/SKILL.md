...
7. **On approval — adapt to the active backend.** Call `lib.active_backend()`:
   - **`social-poster`** (SOCIAL_POSTER_API_KEY set) → react on the specific comment being replied to, pause 8-15s, then post reply with the correct top-level `parentComment` URN.
