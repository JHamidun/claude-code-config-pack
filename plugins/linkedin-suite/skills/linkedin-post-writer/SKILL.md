...
7. **On approval — adapt to the active backend.** Call `lib.active_backend()`:
   - **`social-poster`** (SOCIAL_POSTER_API_KEY set) → schedule via `lib.SocialPublisherClient.create_post` with LinkedIn platformId. If `scheduledTime` omitted, SocialPublisher posts ~90s in the future.
...
- Upstream: `../../your-knowledge-base/linkedin/drafts/` — canonical reference drafts
