---
name: tweaks-panel
description: Плавающая панель "крутилок" внутри прототипа для перебора цветов, шрифтов, отступов, вариантов. Сохраняет состояние (localStorage) и умеет писать значения обратно в исходник (бывший скилл tweaks-persist поглощён — см. references/).
when_to_use: Пользователь просит "сделай это переключаемым", "добавь варианты, которые я смогу прокликать", "хочу сам поэкспериментировать", "сохрани настройки крутилок", "tweaks persist", "запиши выбранные значения в файл / tokens.css".
---
...
- `templates/example.html` — рабочий пример, где меняются цвет, размер шрифта, тёмная тема.
...
Прежняя расширенная версия скилла (дерево @2026-04-30) сохранена целиком в `references/legacy-tweaks-panel.md`. Секции там: Каркас, Использование в App, Какие тёрлы стоит давать, tweaks-persist (если установлен), Антипаттерны.
