...
Без LibreOffice, на Windows с установленным Word — `~/.claude/tools/office_render.py`:
Word сам печатает PDF, pdftoppm режет его на страницы, скрипт отдаёт JSON
`{"pdf": ..., "pngs": [...]}`. Файлы называются `<имя>-docx.pdf` и `<имя>-docx-N.png`.

```bash
python ~/.claude/tools/office_render.py in.docx --out out/ --first 3
```

Файл открывается только на чтение, макросы выключены. По тайм-ауту (`--timeout`,
по умолчанию 180 с) убивается лишь тот Word, который запустил сам скрипт. Word
открывает личную копию документа: если рендер убит посреди открытия, следующий
рендер того же файла не повиснет на вопросе «при последней попытке открыть файл
произошла серьезная ошибка». Нужны `pywin32`, `psutil` и Poppler (`pdftoppm` в
`PATH` или путь в переменной `PDFTOPPM`).
