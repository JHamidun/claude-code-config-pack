---
name: x-post
description: Post to X (Twitter) with auto-threading, images, and videos via your social MCP
---

# X/Twitter Post

Create and schedule posts to X (Twitter) using the your social MCP server. Supports text posts, images (up to 4), videos, and automatic thread splitting for long-form content.
...
1. **Create account** at [your-publisher.example/register](https://your-publisher.example/register)
2. **Choose Pro or Premium plan** (X/Twitter requires paid plan)
3. **Connect X/Twitter** via OAuth in [SocialPublisher Dashboard](https://your-publisher.example/dashboard)
4. **Get API key** at [your-publisher.example/settings/api](https://your-publisher.example/settings/api)
...
📖 **Docs:** [docs.your-publisher.example](https://docs.your-publisher.example)
...
X has specific character counting rules that SocialPublisher handles automatically:
...
When content exceeds 280 characters, SocialPublisher automatically creates a connected thread:
...
|-------|-------|----------|
| "X/Twitter requires Pro plan" | Using Starter plan | Upgrade to Pro at your-publisher.example/settings |
