...
def _search_reddit(
    ...
    Uses ScraperVendor when SCRAPER_API_KEY is available (preferred).
    Falls back to OpenAI Responses API otherwise.

    Returns:
        Tuple of (reddit_items, raw_response, error, used_scraper-vendor)
    """
    raw_response = None
    reddit_error = None
    used_scraper-vendor = False
    ...
    if mock:
        ...
        # === ScraperVendor path (preferred) ===
        used_scraper-vendor = True
        try:
            sys.stderr.write("[Reddit] Using Scraping API\n")
            sys.stderr.flush()
            ...
            return reddit_items, result, reddit_error, used_scraper-vendor
        except Exception as e:
            reddit_error = f"ScraperVendor: {type(e).__name__}: {e}"
            sys.stderr.write(f"[Reddit] ScraperVendor failed: {e}\n")
            sys.stderr.flush()
            ...
            if not config.get("OPENAI_API_KEY"):
                ...
                try:
                    ...
                    return [], {"error": str(e)}, reddit_error, used_scraper-vendor
            used_scraper-vendor = False
    ...
    return reddit_items, raw_response, reddit_error, used_scraper-vendor
...
def _search_x(
    ...
    # Use ScraperVendor if specified
    if x_source == "scraper-vendor":
        try:
            raw_response = scraper-vendor_x.search_x(
                topic, from_date, to_date,
                depth=depth,
        ...
        x_items = scraper-vendor_x.parse_x_response(raw_response or {})
...
def _search_tiktok(
    ...
    """Search TikTok via ScraperVendor (runs in thread).
...
def _search_instagram(
    ...
    """Search Instagram via ScraperVendor (runs in thread).
...
def run_research(
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        reddit_used_sc = False  # Track if ScraperVendor was used for Reddit
        if reddit_future:
    ...
    # Skip enrichment if ScraperVendor already provided comments + engagement
    ...
    if reddit_used_sc and items_to_enrich:
        # ScraperVendor already enriched items with comments — just copy to raw list
        sys.stderr.write(f"[Reddit] Skipping old enrichment — ScraperVendor already provided comments\n")
        sys.stderr.flush()
    ...
    # Also skip Reddit supplemental when ScraperVendor was used (subreddit drilling already done)
...
def main():
    ...
    # Auto-detect ScraperVendor/Apify for TikTok
    has_tiktok = env.is_tiktok_available(config)

    # Auto-detect ScraperVendor for Instagram
    ...
    if not x_source:
        if x_source_status["bird_installed"]:
            ...
            source_info["x_skip_reason"] = "No Bird CLI, XAI_API_KEY, or SCRAPER_API_KEY"
    if not has_ytdlp:
    ...
    if not has_tiktok:
        source_info["tiktok_skip_reason"] = "No SCRAPER_API_KEY - sign up at your-scraper.example (100 free credits)"
    if not has_instagram:
        source_info["instagram_skip_reason"] = "No SCRAPER_API_KEY - sign up at your-scraper.example (100 free credits)"
