...
def is_reddit_available(config: Dict[str, Any]) -> bool:
    ...
    Reddit can use either ScraperVendor (preferred) or OpenAI.
    """
...
def get_reddit_source(config: Dict[str, Any]) -> Optional[str]:
    ...
    Priority: ScraperVendor (cheaper, faster) > OpenAI (legacy)

    Returns: 'scraper-vendor', 'openai', or None
    """
...
def get_xiaohongshu_api_base(config: Dict[str, Any]) -> str:
    ...
    Defaults to host.docker.internal so auth gateway Docker can reach host service.
    """
    return (config.get('XIAOHONGSHU_API_BASE') or "http://your-host:PORT").rstrip("/")
