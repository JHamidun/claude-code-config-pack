---
name: seo-machine-ru
description: "SEO/GEO/AEO контент-машина RU: исследование → написание → оптимизация → публикация лонгридов под Яндекс-стек (Метрика/Вебмастер/Wordstat) и Tilda; семантическое ядро, цитируемость в AI-поиске. Триггеры: «напиши SEO-статью», «оптимизируй под Яндекс», «семантическое ядро», «кластер статей», «операторы вордстат», «попасть в AI-ответы», «GEO/AEO», «почему не ранжируюсь». НЕ: посты→tg-post."
metadata:
  version: 1.1.0
  updated: 2026-06-03
...
**Запуск:**
```bash
docker run -d --name openserp -p 127.0.0.1:7000:7000 --restart unless-stopped karust/openserp:latest serve -a YOUR_PUBLIC_IP -p 7000
```
...
- **Фикс (не сделан, нужны кредды прокси):** `docker run ... karust/openserp:latest serve -a YOUR_PUBLIC_IP -p 7000 --proxy http://user:pass@host:port` (флаг `-x/--proxy` форсит прокси на все движки) или `--2captcha_key <key>` для автоrешения капчи. Для Яндекса нужен именно **РФ-прокси** (резидентский/датацентровый в РФ-подсети).
