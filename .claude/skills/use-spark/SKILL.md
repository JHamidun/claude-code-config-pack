---
name: use-spark
description: "Почта, календарь и контакты из Spark через CLI: письма, треды, поиск, свободные слоты, черновики, встречи. Нужен запущенный Spark Desktop."
metadata:
  version: 1.3.1
  requires:
    bins:
...
## Ошибка «can't access your Spark Desktop application» врёт

Этот текст означает не только «приложение не запущено». То же самое CLI пишет,
когда **запрос упёрся в таймаут 60 секунд** — например, пока Spark
перестраивает поисковый индекс. Проверено 27.08.2026: `spark accounts` работал,
а `spark search` падал с этой ошибкой; в журнале
`~/AppData/Local/Spark Desktop/logs/supportlog-common.txt` стояло настоящее:

```
Db block is executed too long, time: 60.361sec, db: search_fts5.sqlite
CLI command finished: search by timeout [claudeCli] [ok, 61.45s]
```

Прежде чем решать, что приложение недоступно, — посмотреть хвост этого журнала.
Появилось `[Search indexing] Queue drained` — повторить запрос, он пройдёт.
...
целиком и идти по шагам; команды и синтаксис фильтров — выше в этом файле. Первая строка
каждого файла — сводка (имя, версия, уровень доступа). Лицензия MIT, Spark Mail Limited →
`references/LICENSE-spark-cli-skills`. Поля `accessLevel` — уровни из Spark → Settings →
...
|---|---|---|
| [calendar-audit](references/recipes/recipe-calendar-audit.md) | read-only | нагрузка встречами за период: часы, пиковые дни, встречи подряд, свободные окна |
| [delegate-and-track](references/recipes/recipe-delegate-and-track.md) | triage ✎ | назначить письма коллегам с контекстом и следить за статусом делегирования |
| [draft-batch](references/recipes/recipe-draft-batch.md) | triage ✎ | разобрать пачку черновиков: дописать, оставить или выбросить в корзину |
| [end-of-day](references/recipes/recipe-end-of-day.md) | read-only | итог дня: хвосты, закреплённое, календарь на завтра |
| [inbox-by-category](references/recipes/recipe-inbox-by-category.md) | read-only | входящие по умным категориям: priority → люди → приглашения → уведомления → рассылки |
| [inbox-zero](references/recipes/recipe-inbox-zero.md) | triage ✎ | входящие до нуля: по категориям, потом архив, перенос, done, snooze |
| [invitation-manager](references/recipes/recipe-invitation-manager.md) | read-only | приглашения в календарь: сверка занятости и список решений с рекомендацией |
| [label-organize](references/recipes/recipe-label-organize.md) | triage ✎ | метки и папки: навесить, снять ошибочные, перенести не туда положенное |
| [meeting-followup](references/recipes/recipe-meeting-followup.md) | triage, черновики | по расшифровке встречи — черновики писем участникам с action items |
| [meeting-prep](references/recipes/recipe-meeting-prep.md) | read-only | подготовка к встрече: повестка, контекст из писем, участники, открытые треды |
| [morning-standup](references/recipes/recipe-morning-standup.md) | read-only | утренний брифинг: события дня, непрочитанное от людей и priority, назначения команды |
| [multi-account-review](references/recipes/recipe-multi-account-review.md) | read-only | сводка по нескольким ящикам и общие приоритеты |
| [new-sender-review](references/recipes/recipe-new-sender-review.md) | triage ✎ | очередь GateKeeper: принять своих отправителей, заблокировать лишних |
| [newsletter-cleanup](references/recipes/recipe-newsletter-cleanup.md) | triage ✎ | рассылки: блок, отписка, AI-саммари для тех, что оставляем |
| [notification-hygiene](references/recipes/recipe-notification-hygiene.md) | triage ✎ | шумные уведомления: переклассифицировать, сгруппировать, массово архивировать |
| [priority-tuning](references/recipes/recipe-priority-tuning.md) | triage ✎ | настройка категории Priority: ключевые контакты в primary/important |
| [schedule-meeting](references/recipes/recipe-schedule-meeting.md) | read-only | общие свободные слоты участников и варианты времени |
| [shared-inbox-status](references/recipes/recipe-shared-inbox-status.md) | read-only | здоровье общего ящика: open/done, без исполнителя, нагрузка по людям |
| [shared-inbox-triage](references/recipes/recipe-shared-inbox-triage.md) | triage ✎ | разбор общего ящика: назначить, ответить черновиком, закрыть |
| [stakeholder-brief](references/recipes/recipe-stakeholder-brief.md) | read-only | досье на человека: все встречи и переписка с ним в одну справку |
| [team-workload](references/recipes/recipe-team-workload.md) | read-only | распределение назначений в команде: перекосы и неназначенное |
| [topic-timeline](references/recipes/recipe-topic-timeline.md) | read-only | хронология темы: выдержки расшифровок встреч вперемешку с саммари тредов |
| [unsubscribe-audit](references/recipes/recipe-unsubscribe-audit.md) | triage ✎ | полный аудит отписок: три пула, вовлечённость, фишинг, дубли; отписка после одобрения списка |
| [vacation-catchup](references/recipes/recipe-vacation-catchup.md) | triage ✎ | разбор завала после отпуска: массовый архив шума, что требует внимания |
| [weekly-digest](references/recipes/recipe-weekly-digest.md) | read-only | недельная сводка: встречи, непрочитанное по категориям, статус команды |

Персоны (перекрываются со своими навыками `daily-briefing`, `call-prep`, `promise-tracker`):

| Файл | Доступ | Что делает |
|---|---|---|
| [exec-assistant](references/personas/persona-exec-assistant.md) | triage | ассистент руководителя: брифинги, черновики ответов, расписание, контакты |
| [founder](references/personas/persona-founder.md) | triage | основатель/CEO: быстрый триаж, делегирование, надзор за командами |
| [freelancer](references/personas/persona-freelancer.md) | triage | фрилансер: несколько клиентов, напоминания по счетам, доступность |
| [meeting-manager](references/personas/persona-meeting-manager.md) | triage | встречи: подготовка, расшифровки, черновики фоллоу-апов, расписание |
| [project-manager](references/personas/persona-project-manager.md) | triage | проектный менеджер: треды проекта, апдейты стейкхолдерам, action items |
| [sales-rep](references/personas/persona-sales-rep.md) | triage | продажи и аккаунт-менеджмент: клиенты, пайплайн, ритм фоллоу-апов |
| [support-agent](references/personas/persona-support-agent.md) | triage | поддержка: общий ящик, назначения, шаблонные ответы, эскалация |
| [team-lead](references/personas/persona-team-lead.md) | triage | тимлид: нагрузка команды, распределение, подготовка к стендапу |
