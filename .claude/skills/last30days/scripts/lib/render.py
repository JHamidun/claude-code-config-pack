...
def render_compact(report: schema.Report, limit: int = 15, missing_keys: str = "none") -> str:
    ...
    if report.mode == "web-only":
        ...
        lines.append("- `SCRAPER_API_KEY` → Reddit + TikTok + Instagram (one key, all three!) — real upvotes, comments, views")
    ...
    if report.mode == "reddit-only" and missing_keys in ("x", "none"):
        ...
        lines.append("*💡 Tip: Add `SCRAPER_API_KEY` for Reddit + TikTok + Instagram data (one key, all three) and better triangulation.*")
        lines.append("")
