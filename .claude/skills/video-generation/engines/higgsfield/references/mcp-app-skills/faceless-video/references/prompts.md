...
## 2. Assets (seedream_v5_pro, always 1k; pass style key as `image`; {STYLE} verbatim)
...
Each `generate_video_batch.requests[].params` uses model `minimax_h3`,
`duration:10`, `aspect_ratio` = the CHOSEN aspect
(default `"16:9"`; this workflow uses ONLY 16:9 / 9:16), `resolution:"2K"`, `medias` =
location → characters → props (role `image`), **max 7 refs per call** (keep the production packing even though H3 supports more) — only this block's assets; trim extra props/coverage first, never the
...
- **20–23 words, Kids 17–21, comma-light, paced to fill most of the block** (the
  `[00:00-00:09]` timecode carries delivery but does not pace ElevenLabs — see
