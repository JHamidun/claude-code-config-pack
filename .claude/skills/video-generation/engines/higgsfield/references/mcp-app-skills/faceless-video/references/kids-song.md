...
7. **Assemble with `--song`** (Phase 6):
   ...
   sandbox_exec({
     background:true,
     command:"bash ${HF_WORKFLOWS}/faceless-video/scripts/assemble_final.sh --out work/output/final.mp4 --blocks N --manifest clips.txt --song song.wav"
   })
   ```
   ...
   (`"song":true`).
8. Deliver ONE confirmed `final.mp4` through the normal media upload path
