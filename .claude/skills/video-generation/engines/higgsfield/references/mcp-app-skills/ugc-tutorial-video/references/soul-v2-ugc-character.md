...
Wait with `jobs_wait`, then keep `(character_media_id, character_url)` =
`(job_id, result_url)`. That
...
`generate_image_batch` with `model: "soul_2"`, `aspect_ratio: "3:4"`, `quality: "2k"` — the prompt string exactly as built above, no JSON wrapper. Poll
`jobs_wait` to a terminal state and carry `(job_id, result_url)` forward as the character
