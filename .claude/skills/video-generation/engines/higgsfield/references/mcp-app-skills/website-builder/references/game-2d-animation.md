...
| 6 | Alpha | ffmpeg chroma key, per frame | transparent-background PNGs |
...
- **The video model is `seedance_1_5` exactly.** Not `seedance_2_0`, not the
  ...
  `models_get` on the model id if a call errors, but do not silently substitute a model.
...
## Stage 6 — Alpha (per frame, chroma key)

This surface has no background-remover tool, so the alpha is planned at
generation time instead of recovered afterwards — which is how sprite pipelines
work anyway, and it is more reliable than an AI matte on a 25-frame sequence.

**Stage 2 and 3 must put the subject on a flat chroma ground.** Append to the
key-pose and video prompts: "on a completely flat solid #00FF00 background, even
lighting, no shadow on the background, no props, no scenery". A green that
appears in the character's own palette forces a different key colour — use flat
magenta `#FF00FF` for anything green.

Then key every frame in one `sandbox_exec` command, straight out of the video:

```bash
ffmpeg -i anim.mp4 -vf "chromakey=0x00FF00:0.12:0.04,despill" -pix_fmt rgba frame_%03d.png
```

Widen the similarity (`0.12`) only until the ground is gone; past that it starts
eating the subject. Inspect one mid-sequence frame at full size before assembling
the sheet — a halo or a chewed silhouette is far cheaper to catch here than after
the grid is built.

**Why per-frame and not a single video-wide matte:** each frame is keyed
independently, so edges stay crisp. A temporal matte optimizes for smoothness and
smears edges across frames, which on sprites reads as a flickering halo.
