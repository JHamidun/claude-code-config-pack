...
**Tools.** They are async — **submit → `jobs_wait` → collect with one
`show_generation_by_ids` → use the result**. Independent renders go out together
as one `generate_image_batch` / `generate_video_batch`. If a param/model is
rejected, call `models_get` on the model id and use what it reports.
...
There is no background remover and no image-to-3D on this surface. For a
transparent cutout, prompt the subject "isolated on a plain flat #00FF00
background, no shadow" and key it out in `sandbox_exec`
(`magick in.png -fuzz 12% -transparent '#00FF00' out.png`). For anything that
wanted a mesh, use the layered-depth alternative in `references/wow-catalog.md`.
...
- **Product cutouts** — `generate_image` on a flat chroma ground → key it out in
  the sandbox → transparent PNG.
- **Layered depth** — the 2D stand-in for a 3D subject: cut the subject onto its
  own layer and parallax it against a generated background plate.
...
cutouts ~800px); for a monochrome look force grayscale in the prompt AND on
export.
...
- **Hold-to-spin turntable** `[W]` — a clay/toy-look turntable, driven by a
  generated frame sequence rather than a mesh: render the subject at 24-36 evenly
  spaced yaw angles with one `generate_image_batch`, then scrub the sequence on
  drag with inertia and click hotspots for detail. *Build:* plain canvas or
  `<img>` swapping — no `three`, since this surface cannot generate a mesh.
