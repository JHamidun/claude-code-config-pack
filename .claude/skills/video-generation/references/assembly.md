...
Альтернатива ducking'у (§3/§4): когда закадр — это **N отдельных реплик по сценам** поверх одного оркестрового трека, и pumping sidechain мешает интимной подаче. Каждую реплику кладём по таймкоду её сцены через `adelay`, музыку держим тихо и ровно. Это рецепт «Хроник Восхождения» (`assemble.py`), давший чистый mix:
