# Higgsfield-derived flows & techniques (RE 2026-06-07)

Техники, снятые реверсом Higgsfield Supercomputer (наблюдением за их employees), переложенные на наш пайплайн.
...
Вшивать в КАЖДЫЙ промпт серии (их главный приём консистентности — cinematic-flow Soul-anchor + classicMD LOCKS):
```
...
дёшево ревьюить раскадровку до дорогих видео-генераций. Шаблон листа (их `classicMD-board`) →
...
## Prompt-схемы (их флоу) → шаблоны для Seedance/Veo
...
## Звук
Procedural BGM чистым ffmpeg (бесплатная подложка под превью) → `video-editor/references/procedural-bgm.md`.
...
(иначе всегда Runway $0). Цены hf: 720p 4.5 cr/s (fast 3.5), 1080p 9. Virality-проверка финала: `hf generate create brain_activity --video`.
