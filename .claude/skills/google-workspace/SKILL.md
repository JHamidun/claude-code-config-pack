...
description: "Google Workspace и рабочая почта одним набором инструментов: Google Docs (прочитать документ, найти документ, дописать текст), Google Sheets (прочитать таблицу, записать, найти таблицу, листы), Gmail (личная почта: письмо, отправить, найти, непрочитанные, вложения, 20 ящиков), Google Drive (файл на диске, папка, скачать, расшаренная папка, общий диск), Outlook/Exchange (рабочая почта компании через локальный Outlook). Триггеры: «гугл документ», «прочитай документ», «саммари документа», «прочитай таблицу», «запиши в таблицу», «гугл таблица», «письмо», «отправь письмо», «непрочитанные письма», «личная почта», «рабочая почта», «письма компании», «файл на диске», «гугл диск», «скачай с диска», «что в папке на диске». НЕ: Яндекс.Диск и Яндекс.Почта → skill yandex; произвольный IMAP-ящик → skill email-imap; Google Календарь/Контакты/Задачи/Аналитика → команды /gcalendar /gcontacts /gtasks /ganalytics; перевод → skill deepl-pro."
---
...
| подключить новый Gmail-ящик | `python ~/.claude/tools/gmail_multi_auth.py <ящик>` |
...
расшарил её на робота `claude-code-main@claude-code-481818.iam.gserviceaccount.com`,
...
python ~/.claude/skills/google-workspace/scripts/outlook_local.py search "Воложанин" --field from
...
| Задача | Куда |
| --- | --- |
| Яндекс.Диск, Яндекс.Почта, Метрика, Директ | skill `yandex` |
