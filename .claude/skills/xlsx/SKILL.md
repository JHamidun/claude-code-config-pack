...
Посмотреть посчитанное, не трогая файл, на Windows с Excel можно через
`python ~/.claude/tools/office_render.py book.xlsx --out out/`: Excel пересчитывает
формулы и печатает все листы в PDF и PNG (`<имя>-xlsx-N.png`), а сам `.xlsx`
остаётся нетронутым. Значения видны только на картинках, автоматического поиска
`#REF!`/`#DIV/0!` нет — ошибки ищите глазами.
