...
trigger: "\"проверь прайс\", \"латиница в прайсе\", \"проверка каталога\", \"смешение кириллицы\", \"latin check\", \"прайс-лист проверка\""
---
...
```bash
python ~/.claude/skills/pricelist-latin-check/scripts/latin_check.py <path/to/file.xlsx>

# только критичные опечатки, без «английских слов»
python ~/.claude/skills/pricelist-latin-check/scripts/latin_check.py <file.xlsx> --only-critical

# свой словарь допустимой латиницы (по одному слову в строке)
python ~/.claude/skills/pricelist-latin-check/scripts/latin_check.py <file.xlsx> --whitelist my-brands.txt

# конкретные колонки вместо авто-определения
python ~/.claude/skills/pricelist-latin-check/scripts/latin_check.py <file.xlsx> --columns E,F,S
```
