...
**Перед началом прочитай `~/.claude/business-context.md`** — разделы «Продукт», «ICP», «Цены» (действующая линейка и валюта), «Экономика» (средний чек, CAC, LTV, отток) и «Позиционирование» (с кем тебя сравнивают). Файла нет — заведи из `~/.claude/templates/business-context.md`.
