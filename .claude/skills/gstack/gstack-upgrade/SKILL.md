...
[ -z "$_AUTO" ] && _AUTO=$(bash ~/.claude/skills/gstack/bin/gstack-config get auto_upgrade 2>/dev/null || true)
...
```
Tell user: "Update checks disabled. Run `bash ~/.claude/skills/gstack/bin/gstack-config set update_check true` to re-enable."
...
```bash
bash ~/.claude/skills/gstack/bin/gstack-update-check --force
```
