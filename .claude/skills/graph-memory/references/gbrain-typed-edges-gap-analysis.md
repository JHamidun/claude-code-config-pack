...
> Цель — усилить НАШ локальный граф памяти `~/.claude/scripts/memory_graph.py` (`~/.claude/memory-graph/graph.db`).
...
| Граф-сигнал даёт **+31.4pp P@5** vs vector-only/BM25 | ретрив у нас в Second Brain (`~/.brain`), не в memory_graph | обоснование, зачем граф-слой в гибридном ретриве (не код-правка тут) |
...
```
Куда встроить в поток: секция dream (`skills/dream`) — `gaps` даёт готовый to-do список консолидации (что вплести, что пометить superseded, что проверить на дрейф). Роутинг «висячие ссылки/хабы/orphans» уже указывает на memory_graph.
...
gbrain +31.4pp P@5 получен добавлением graph adjacency boost к vector+BM25. Наш ретрив — в Second Brain (`~/.brain`). Если/когда там появится реранкинг — adjacency из `graph.db` (сосед-хабы искомого) можно подмешивать как boost. Heavy-job, под ресурс-гард, здесь только заметка.
