...
- **`api.bgpview.io` не резолвится с этой машины**, поэтому основной источник BGP — RIPEstat. Это учтено, менять не нужно.
...
- `requests` и `dnspython` используются как быстрый путь, если установлены (оба есть); иначе всё работает на stdlib.
...
**Живые карты самолётов, судов и спутников — не в этом скилле.** Они требуют постоянного опроса (поллер каждые N секунд, фоновый демон, хранилище треков), что противоречит философии `osint-recon`: спросил → сходил → напечатал. Это **не значит «такого нет»** — под живую ситуационную осведомлённость есть отдельный скилл **`shadowbroker-map`** (ADS-B, AIS, спутники, GDELT; управляется `shadowbroker_ctl.py {start|stop|status}` под ресурс-гардом). Туда и идти, если вопрос звучит как «что летает над», «покажи живую карту», «военные борта рядом».
...
| Живая обстановка: самолёты, суда, спутники, события в реальном времени | `shadowbroker-map` |
| Обогащение лидов, ЕГРЮЛ, цифровой след компании, квалификация заявок | `lead-enrichment` |
| Досье по соцсетям, профили людей, KYC по цифровому следу | `social-intel` |
| Проверка контрагентов РФ (ФНС, реквизиты, арбитраж) | `fns-check`, `garant-research` |
| Аудит СВОЕГО кода и инфраструктуры, OWASP, уязвимости в репо | `security-audit`, `threat-hunting`, агент `security-scanner` |
| Обезличивание перед публикацией, поиск PII в своих файлах | `leak-scan`, `privacy-filter` |
