...
description: "Сохраняет персональную базу знаний (инструкцию, справочник, документацию) в 4 уровня памяти, чтобы будущие сессии нашли её по триггерам. Триггеры: «запомни эту инструкцию», «сохрани в память», «чтобы не забыл», «сделай чтобы при запросе X сразу выдавал», «сохрани знание о», «добавь в постоянную память». НЕ для эфемерных заметок — для долговременных reference-документов (мануалы, справки, досье)."
