...
metadata:
  version: 1.0.0
  type: analysis
  triggers: ["session mentor","как я работаю","проанализируй сессии","отчёт по работе","где теряю время","mentor отчёт"]
---
...
- **Кросс-линк с memory_graph**: `python ~/.claude/scripts/memory_graph.py hubs` / `search <тема>` — превратить `session_themes` в реальные проекты/инструменты/людей (у mentor этого нет).
