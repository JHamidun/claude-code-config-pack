---
name: session-mentor
description: "Локальный анализ истории сессий Claude Code (и Codex) → HTML-отчёт «как ты работаешь»: темы, использование инструментов, успехи, точки трения, недоиспользуемые возможности, правки для CLAUDE.md, новые сценарии, тренды. Считает метрики (сессии/сообщения/tool-calls/правки/файлы/токены). Данные не покидают машину. Триггеры: «session mentor», «как я работаю с claude code», «проанализируй мои сессии», «отчёт по работе», «где я теряю время», «статистика сессий за N дней». НЕ: полнотекстовый поиск по чатам→/search-chats; статистика ОДНОЙ сессии→/prompt-log."
metadata:
  version: 1.0.0
  type: analysis
