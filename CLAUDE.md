...
- **Навык вместо ad-hoc — когда задача попадает в его описание.** Не «зови навык под любую задачу»: бланкетный дефолт даёт перетриггеривание («Instructions like "If in doubt, use [tool]" will cause overtriggering»), а описания навыков и так лежат в промпте и находятся сами. Карта на случай, когда описание не помогло: `rules/routing.md` (ходовые триггеры) → `config/routing-ext.md` (полная карта всех триггеров, Read по требованию).
...
| **Почему навыки «не находятся» (бюджет листинга)** | `config/skill-listing-budget.md` |
| **Производительность: процессы, хуки, MCP** | `config/performance.md` |
...
314 готовых скиллов `skills/` · 74 агента `agents/` (57 в корне + 17 в подкаталогах health/meta/testing) · 155 команд `commands/` (98 в корне + 57 в gsd/) · 18 правил `rules/` (ядро) + `config/rules-ref/` (справочники) · 33 плагина включено (каталог → `config/rules-ref/plugins-catalog.md`).

> Что считается единицей: скилл — каталог со `SKILL.md`; агент, команда, правило — файл `.md`, **кроме `README.md`** (индекс раздела — не агент и не команда). Определение записано здесь потому, что три инструмента считали по-разному и получали 75/156/19 против 74/155/18: расхождение ровно на три индексных файла выглядело как ошибка счёта, а было разночтением в том, что вообще считать.

> Числа выше — срез на 2026-08-23; они дрейфуют с каждым обновлением пака. Актуальные цифры пересчитывай через `scripts/config_lint.py` (он смотрит на `CLAUDE_CONFIG_DIR`, иначе на `~/.claude`), не цитируй по памяти: прошлый срез разошёлся с диском на восемь агентов ровно потому, что его написали руками.
