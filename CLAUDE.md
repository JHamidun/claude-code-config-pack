...
Навыки `skills/` · агенты `agents/` (root + workers) · команды `commands/` · правила `rules/` (ядро) + `config/rules-ref/` (справочники) · плагины (каталог → `config/rules-ref/plugins-catalog.md`).

> Не полагайся на счётчики по памяти — они дрейфуют. Актуальные цифры всегда бери из `scripts/config_lint.py`.
