...
287 готовых скиллов `skills/` · 74 агента `agents/` (57 в корне + 17 в подкаталогах health/meta/testing) · 155 команд `commands/` (98 в корне + 57 в gsd/) · 18 правил `rules/` (ядро) + `config/rules-ref/` (справочники) · 33 плагина включено (каталог → `config/rules-ref/plugins-catalog.md`).
...
> Числа выше — срез на 2026-08-18; они дрейфуют с каждым обновлением пака. Актуальные цифры пересчитывай через `scripts/config_lint.py` (он смотрит на `CLAUDE_CONFIG_DIR`, иначе на `~/.claude`), не цитируй по памяти: прошлый срез разошёлся с диском на восемь агентов ровно потому, что его написали руками.
