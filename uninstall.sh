...
#   2) он лежит внутри ~/.claude (или это ~/CLAUDE.md, который создали мы);
#   3) он не симлинк;
...
тобой после установки. Ключи, память, история сессий, локальные настройки, ~/CLAUDE.md
(если он был твоим) и резервные копии не трогаются.
...
while IFS="$TAB" read -r sz mt p; do
    ...
    [ -n "$p" ] || continue
    # Путь обязан быть внутри ~/.claude либо быть ровно ~/CLAUDE.md. Никаких "..",
    # никаких абсолютных путей — манифест мог быть подправлен руками.
    case "$p" in
